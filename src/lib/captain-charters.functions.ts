...
const boatSelect = "name,make,model,length_ft,capacity,home_port,description,hero_image_url,image_urls";
...
export const listCaptainCharters = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: rows, error } = await context.supabase
      .from("charters")
      .select(`id,slug,name,hero_url,description,water_type,target_species,boat_id,boat:${boatSelect},is_published,base_price_cents,duration_minutes,capacity,created_at,packages:bookable_services(id,title,hero_url,base_price_cents,capacity,duration_minutes,is_published,is_bookable,service_kind)`)
